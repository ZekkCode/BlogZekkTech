...
@section('content')
    ...
    <div class="hero-section relative w-full h-64 md:h-80 overflow-hidden rounded-xl mb-8 sm:mb-12 card">
        ...
        <div
            class="absolute inset-0 bg-gradient-to-r from-black/60 to-transparent flex flex-col justify-center items-start text-left p-4 sm:p-8">
            ...
            @if($posts->count() > 0)
                <div class="space-y-6 sm:space-y-8">
                    @foreach($posts as $post)
                        <article
                            class="card rounded-xl p-4 sm:p-6 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
                            <div class="flex flex-col md:flex-row gap-4 sm:gap-6">
                                ...
                                <div class="flex-1">
                                    ...
                                    @if($post->excerpt)
                                        <p class="mb-3 sm:mb-4 leading-relaxed text-sm sm:text-base font-medium"
                                        ...
                                        <div class="flex items-center gap-2">
                                            @if($post->user->avatar)
                                                <img src="{{ $post->user->avatar }}" alt="{{ $post->user->name }}"
                                                    class="w-6 h-6 sm:w-8 sm:h-8 rounded-full object-cover border"
                ...
                <div class="card rounded-xl p-6 sm:p-12 text-center">
                    <div class="max-w-md mx-auto">
                        <div class="w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6 rounded-full flex items-center justify-center"
                            ...
                            <svg class="w-8 h-8 sm:w-12 sm:h-12" style="color: var(--text-secondary);" fill="none"
                                stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.746 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                            </svg>
                        </div>
                        <h3 class="text-lg sm:text-xl font-semibold mb-2" style="color: var(--text-primary);">TIDAK ADA
                            POSTINGAN</h3>
